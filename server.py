...
# ============ AI自主学习模块 ============
AI_LEARNING_FILE = 'ai_learning_data.pkl'

class AISelfPlay:
    """AI自我对弈学习模块 - 在空闲时间自我对弈提升棋力"""
    
    def __init__(self):
        self.running = False
        self.thread = None
        self.learning_data = {
            'positions': {},      # 位置特征 -> {'wins': 0, 'total': 0}
            'opening_book': {},   # 棋盘hash -> {位置: 胜率}
            'move_history': [],   # 最近对弈历史
            'total_games': 0,
            'black_wins': 0,
            'white_wins': 0,
    ...
    def load_learning_data(self):
        ...
        try:
            if os.path.exists(AI_LEARNING_FILE):
                with open(AI_LEARNING_FILE, 'rb') as f:
                    self.learning_data = pickle.load(f)
                print(f"✅ 加载学习数据: {self.learning_data.get('total_games', 0)} 局")
    ...
    def save_learning_data(self):
        """保存学习数据"""
        try:
            with open(AI_LEARNING_FILE, 'wb') as f:
    ...
    def get_board_hash(self, board):
        """将棋盘转换为可哈希的字符串"""
        return ''.join(str(cell) for cell in board for cell in cell)
    
    def get_position_key(self, board, row, col, color):
        """获取位置特征键 - 提取局部特征"""
        key = f"{row},{col},{color}"
        # 提取周围5x5区域特征
        for dr in range(-2, 3):
    ...
    def evaluate_with_learning(self, board, row, col, color):
        """使用学习数据辅助评估位置"""
        base_score = evaluate_position(board, row, col, color)
        
        # 查询学习数据
        key = self.get_position_key(board, row, col, color)
        stats = self.learning_data['positions'].get(key, {'wins': 0, 'total': 0})
        
        if stats['total'] > 5:  # 至少5次数据才有效
            win_rate = stats['wins'] / stats['total']
            # 根据学习数据调整分数: 胜率高则加分，低则减分
            learning_boost = (win_rate - 0.5) * 1.5
            base_score *= (1 + learning_boost * 0.4)
        
        return base_score
    
    def get_ai_move_with_learning(self, board, color):
        """使用学习数据辅助决策的AI走法"""
        best_score = -1
        best_moves = []
        
        # 检查棋盘是否为空
        empty_count = sum(1 for r in range(BOARD_SIZE) for c in range(BOARD_SIZE) if board[r][c] == 0)
        ...
        if board_hash in self.learning_data['opening_book']:
            opening_moves = self.learning_data['opening_book'][board_hash]
            if opening_moves:
                # 按胜率排序选择最佳开局
                sorted_moves = sorted(opening_moves.items(), key=lambda x: x[1], reverse=True)
                for pos_key, win_rate in sorted_moves[:3]:
                    try:
                        r, c = map(int, pos_key.split(','))
                        if board[r][c] == 0:
                            # 有概率选择开局库走法（增加探索）
    ...
    def record_game_result(self, board, moves, winner):
        """记录对局结果用于学习"""
        with self.game_lock:
            self.learning_data['total_games'] += 1
            if winner == 1:
                self.learning_data['black_wins'] += 1
            elif winner == 2:
                self.learning_data['white_wins'] += 1
            else:
                self.learning_data['draws'] += 1
            
            # 记录每个位置的胜负
            for idx, (row, col, color) in enumerate(moves):
                # 只有赢家才记录正面数据，输家记录负面数据
                is_winner = (color == winner) if winner else False
                key = self.get_position_key(board, row, col, color)
                
                if key not in self.learning_data['positions']:
                    self.learning_data['positions'][key] = {'wins': 0, 'total': 0}
                
                self.learning_data['positions'][key]['total'] += 1
                if is_winner:
                    self.learning_data['positions'][key]['wins'] += 1
            
            # 记录开局库
            if len(moves) >= 4:
                for i in range(0, min(len(moves), 8), 2):
                    partial_board = [[0 for _ in range(BOARD_SIZE)] for _ in range(BOARD_SIZE)]
                    for j in range(i):
                        r, c, col = moves[j]
                        partial_board[r][c] = col
                    
                    board_hash = self.get_board_hash(partial_board)
                    if board_hash not in self.learning_data['opening_book']:
                        self.learning_data['opening_book'][board_hash] = {}
                    
                    # 记录下一个走法的胜率
                    if i < len(moves):
                        next_r, next_c, next_col = moves[i]
                        pos_key = f"{next_r},{next_c}"
                        if pos_key not in self.learning_data['opening_book'][board_hash]:
                            self.learning_data['opening_book'][board_hash][pos_key] = 0.5
                        
                        # 更新胜率
                        total = self.learning_data['opening_book'][board_hash][pos_key] * 10 + (1 if is_winner else 0)
                        self.learning_data['opening_book'][board_hash][pos_key] = total / 11
            
            # 限制数据量防止内存溢出
            if len(self.learning_data['positions']) > 100000:
                # 保留最常用的数据
                sorted_positions = sorted(
                    self.learning_data['positions'].items(),
                    key=lambda x: x[1]['total'],
                    reverse=True
                )[:50000]
                self.learning_data['positions'] = dict(sorted_positions)
            
            if len(self.learning_data['opening_book']) > 10000:
                sorted_book = sorted(
                    self.learning_data['opening_book'].items(),
                    key=lambda x: sum(x[1].values()),
                    reverse=True
                )[:5000]
                self.learning_data['opening_book'] = dict(sorted_book)
    ...
    def play_self_game(self):
        ...
        moves = []
        current_color = 1  # 黑棋先手
        move_count = 0
        
        while move_count < BOARD_SIZE * BOARD_SIZE:
            row, col = self.get_ai_move_with_learning(board, current_color)
            ...
            moves.append((row, col, current_color))
            move_count += 1
            
            # 检查胜负
            if check_win(board, row, col, current_color):
                self.record_game_result(board, moves, current_color)
        ...
        # 平局
        self.record_game_result(board, moves, None)
        return None
    ...
    def self_play_loop(self):
        ...
        while self.running:
            try:
                ...
                # 执行自我对弈
                result = self.play_self_game()
                game_count += 1
                
                if game_count % 10 == 0:
                    print(f"📊 AI已完成 {self.learning_data['total_games']} 局自我对弈")
                    print(f"  黑胜: {self.learning_data['black_wins']}, 白胜: {self.learning_data['white_wins']}, 平局: {self.learning_data['draws']}")
                    print(f"  位置数据: {len(self.learning_data['positions'])} 条")
                    print(f"  开局库: {len(self.learning_data['opening_book'])} 个局面")
    ...
    def start(self):
        ...
        self.running = True
        self.thread = threading.Thread(target=self.self_play_loop, daemon=True)
        self.thread.start()
        print("🤖 AI自主学习已启动")
    ...
    def get_stats(self):
        """获取学习统计信息"""
        return {
            'total_games': self.learning_data['total_games'],
            'black_wins': self.learning_data['black_wins'],
            'white_wins': self.learning_data['white_wins'],
            'draws': self.learning_data['draws'],
            'position_count': len(self.learning_data['positions']),
            'opening_count': len(self.learning_data['opening_book'])
        }

# 创建全局AI学习实例
ai_self_play = AISelfPlay()

# ============ 修改AI走法函数以使用学习数据 ============
def ai_find_best_move(board, color):
    """AI寻找最佳落子位置 - 使用学习数据增强"""
    return ai_self_play.get_ai_move_with_learning(board, color)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ai-learning-stats { background: #0f0f23; border-radius: 8px; padding: 6px 10px; margin-top: 4px; font-size: 11px; color: #888; border: 1px solid #00d4ff33; }
        .ai-learning-stats .highlight { color: #00d4ff; }
        .ai-learning-stats .highlight2 { color: #ffd93d; }
    </style>
</head>
<body>
...
<div class="container" id="mainApp">
    <div class="header">
        <h1>♟ 五子棋 <span style="font-size:12px;color:#00d4ff;">🧠AI自主学习</span></h1>
        <div>
    ...
    <div class="main">
        ...
        <div class="side-panel">
            ...
            <div id="adminPanel" class="admin-panel hidden">
                ...
                <div class="title" style="margin-top:8px;">🧠 AI学习状态</div>
                <div class="ai-learning-stats" id="aiLearningStats">
                    <span class="highlight">🤖</span> 自主学习进行中...
                </div>
            </div>
            <div class="chat-box">
...
const aiLearningStats = document.getElementById('aiLearningStats');
...
// ========== 接收AI学习状态 ==========
socket.on('ai_learning_stats', (data) => {
    if (data) {
        aiLearningStats.innerHTML = `
            🤖 已学习 <span class="highlight">${data.total_games}</span> 局 | 
            黑胜 <span class="highlight">${data.black_wins}</span> | 
            白胜 <span class="highlight">${data.white_wins}</span> | 
            平局 <span class="highlight2">${data.draws}</span> |
            位置特征 <span class="highlight">${data.position_count}</span> 条 |
            开局库 <span class="highlight">${data.opening_count}</span> 个
        `;
    }
});
...
// 定时请求学习状态
setInterval(() => {
    socket.emit('get_ai_learning_stats');
}, 5000);
...
socket.on('room_joined', (data) => {
    ...
    if (state.isAiRoom && !state.isWatcher && state.color === 1) {
        addChat('系统', '🤖 你执黑先手，xp12喵的AI执白后手 (AI已通过自主学习增强)');
    }
    drawBoard();
    ...
    if (!state.gameOver) {
        ...
        if (state.isWatcher) {
            ...
            if (state.isAiRoom && state.color === 2 && state.currentTurn === 2) {
                addChat('系统', '🤖 xp12喵的AI 正在思考... (已学习 ' + (state.aiLearningCount || '?') + ' 局)');
            }
        }
    } else {
...
socket.on('game_over', (data) => {
    ...
    if (winner) {
        if (winner === 'xp12喵的AI') {
            ...
            addChat('系统', '🤖 xp12喵的AI 赢了！AI通过自主学习又变强了！');
        } else {
...
function drawBoard() {
    ...
    if (state.lastMove) {
        ...
        ctx.shadowBlur = 0;
        ctx.beginPath();
        ctx.arc(x, y, CELL * 0.46, 0, 2 * Math.PI);
        ctx.stroke();
        ctx.shadowBlur = 0;
    }
}
...
console.log('🎯 五子棋联机版已加载 - AI自主学习版');
console.log('🤖 AI正在后台自我对弈提升棋力...');
...
def handle_create_room(data):
    ...
    broadcast_rooms()
    if is_ai:
        socketio.emit('chat', {"username": "系统", "message": f"🤖 {username} 创建了AI练习房间 '{clean_name}'，你执黑先行，xp12喵的AI执白 (AI已自主学习增强)"}, to=room_id)
    else:
...
def handle_place_piece(data):
    ...
    if check_win(room["board"], row, col, color):
        ...
        if room.get("is_ai_room", False):
            ...
            socketio.emit('chat', {"username": "xp12喵的AI", "message": "你赢了！我会继续学习的！💪"}, to=room_id)
        else:
        ...
        if room.get("is_ai_room", False) and len(players) >= 1:
            for p in players:
                if p != "xp12喵的AI":
                    ...
                    update_ai_ranking("xp12喵的AI", 'draw')
                    break
            socketio.emit('chat', {"username": "xp12喵的AI", "message": "平局了！你很强！🤝"}, to=room_id)
...
def handle_resign(room_id):
    ...
    if room.get("is_ai_room", False):
        ...
        socketio.emit('chat', {"username": "xp12喵的AI", "message": "认输了？下次加油！😊"}, to=room_id)
    elif winner:
...
# ========== AI学习状态查询 ==========
@socketio.on('get_ai_learning_stats')
def handle_get_ai_learning_stats():
    sid = request.sid
    username = clients[sid].get("username")
    if not username or not is_admin(username):
        return
    stats = ai_self_play.get_stats()
    emit('ai_learning_stats', stats)
