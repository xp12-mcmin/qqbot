...
# ============ 防刷分机制（真人房间互刷） ============
# 限制真人房间每30分钟最多15局
MAX_PVP_GAMES_PER_30MIN = 15
PVP_WINDOW_SECONDS = 1800  # 30分钟

# 记录玩家的PVP对局时间
player_pvp_game_times = defaultdict(list)  # {username: [timestamp1, timestamp2, ...]}

def check_pvp_game_limit(username):
    """
    检查玩家在真人房间的对局是否超过限制
    返回: (是否允许, 错误信息)
    """
    if not username or username == "xp12喵的AI":
        return True, ""
    
    current_time = time.time()
    window_start = current_time - PVP_WINDOW_SECONDS
    
    # 清理30分钟前的记录
    player_pvp_game_times[username] = [t for t in player_pvp_game_times[username] if t > window_start]
    
    # 检查是否超过限制
    if len(player_pvp_game_times[username]) >= MAX_PVP_GAMES_PER_30MIN:
        return False, f"⏳ 真人房间30分钟内已进行 {MAX_PVP_GAMES_PER_30MIN} 局，请稍后再玩（AI房间不限）"
    
    return True, ""

def record_pvp_game(username):
    """记录玩家完成了一局真人PVP游戏"""
    if username and username != "xp12喵的AI":
        player_pvp_game_times[username].append(time.time())
        # 只保留最近100条
        if len(player_pvp_game_times[username]) > 100:
            player_pvp_game_times[username] = player_pvp_game_times[username][-100:]

def reset_pvp_records(username):
    """重置玩家的PVP记录（用于封禁后清理）"""
    if username in player_pvp_game_times:
        del player_pvp_game_times[username]
...
def update_ranking(winner, loser=None, room_id=None):
    """更新排行榜 - 带PVP对局限制（真人房间每30分钟最多15局）"""
    if winner == "xp12喵的AI" or loser == "xp12喵的AI":
        return
    
    # 检查赢家是否超过限制
    can_play, msg = check_pvp_game_limit(winner)
    if not can_play:
        print(f"[防刷分] 阻止排名更新: {winner} - {msg}")
        if room_id:
            socketio.emit('chat', {
                "username": "系统", 
                "message": f"⚠️ {winner} {msg}"
            }, to=room_id)
        return
    
    # 检查输家是否超过限制（输家也要限制，防止送分）
    if loser:
        can_play_loser, msg_loser = check_pvp_game_limit(loser)
        if not can_play_loser:
            print(f"[防刷分] 输家超过限制: {loser} - {msg_loser}")
            if room_id:
                socketio.emit('chat', {
                    "username": "系统", 
                    "message": f"⚠️ {loser} {msg_loser}"
                }, to=room_id)
            # 输家超限，只记录赢家的胜场，但不记录输家的败场
            ranking = load_ranking()
            winner_clean = sanitize_input(winner, MAX_USERNAME_LEN)
            if winner_clean not in ranking:
                ranking[winner_clean] = {'wins': 0, 'losses': 0, 'draws': 0, 'total': 0}
            ranking[winner_clean]['wins'] += 1
            ranking[winner_clean]['total'] += 1
            save_ranking(ranking)
            broadcast_ranking()
            record_pvp_game(winner_clean)
            return
    ...
    # 记录PVP对局
    record_pvp_game(winner)
    if loser:
        record_pvp_game(loser)
...
def update_ranking_draw(player1, player2, room_id=None):
    """平局更新 - 带PVP对局限制"""
    if player1 == "xp12喵的AI" or player2 == "xp12喵的AI":
        return
    
    # 检查双方是否超过限制
    can_play, msg = check_pvp_game_limit(player1)
    if not can_play:
        if room_id:
            socketio.emit('chat', {"username": "系统", "message": f"⚠️ {msg}"}, to=room_id)
        return
    
    can_play2, msg2 = check_pvp_game_limit(player2)
    if not can_play2:
        if room_id:
            socketio.emit('chat', {"username": "系统", "message": f"⚠️ {msg2}"}, to=room_id)
        return
    ...
    if not room.get("game_over", False) and username_clean in room.get("player_colors", {}):
        ...
        if winner:
            ...
            if not can_play:
                ...
                # 检查离开者是否也在刷分
                if username_clean:
                    can_loser, _ = check_pvp_game_limit(username_clean)
                    if can_loser:
                        update_ranking(winner, username_clean, room_id)
                    else:
                        # 只更新赢家，不记录输家（输家已超限）
                        ranking = load_ranking()
                        winner_clean = sanitize_input(winner, MAX_USERNAME_LEN)
                    ...
                    update_ranking(winner, None, room_id)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .pvp-limit-notice { background: #ffd93d22; border: 1px solid #ffd93d; color: #ffd93d; padding: 4px 10px; border-radius: 4px; font-size: 12px; text-align: center; margin-bottom: 4px; display: none; }
    </style>
</head>
<body>
    ...
    <div class="main">
        ...
        <div class="side-panel">
            <div>
                ...
                <div class="pvp-limit-notice" id="pvpLimitNotice">⏳ 真人房间每30分钟限15局，AI房间不限</div>
            ...
            <div id="adminPanel" class="admin-panel hidden">
                ...
                <div class="title" style="margin-top:8px;">📊 防刷分监控</div>
                <div style="display:flex;gap:4px;">
                    <input type="text" id="checkPvpInput" placeholder="查看玩家对局记录..." style="flex:1;padding:3px 6px;border-radius:4px;border:none;background:#1a1a2e;color:#fff;font-size:12px;">
                    <button id="checkPvpBtn" style="padding:3px 10px;border-radius:4px;border:none;background:#00d4ff;color:#1a1a2e;cursor:pointer;font-size:12px;">查询</button>
                </div>
                <div id="pvpCheckResult" style="font-size:11px;color:#888;margin-top:4px;max-height:60px;overflow-y:auto;"></div>
            </div>
            <div class="chat-box">
...
const pvpLimitNotice = document.getElementById('pvpLimitNotice');
const checkPvpInput = document.getElementById('checkPvpInput');
const checkPvpBtn = document.getElementById('checkPvpBtn');
const pvpCheckResult = document.getElementById('pvpCheckResult');
...
socket.on('login_success', (data) => {
    ...
    socket.emit('get_ranking');
    // 显示PVP限制提示
    pvpLimitNotice.style.display = 'block';
});
...
socket.on('pvp_player_info', (data) => {
    if (!data) return;
    pvpCheckResult.innerHTML = `
        <div style="color:#00d4ff;">📊 ${data.username}</div>
        <div>30分钟内对局: <strong>${data.games_30min}</strong> / 15局</div>
        <div>总对局数: ${data.total_games}</div>
        ${data.games_30min >= 15 ? '<div style="color:#ff6b6b;">⚠️ 已超过限制，无法继续PVP</div>' : '<div style="color:#00ff88;">✅ 可正常对局</div>'}
        ${data.recent_times && data.recent_times.length > 0 ? '<div style="font-size:10px;color:#666;">最近: ' + data.recent_times.join(', ') + '</div>' : ''}
    `;
});
...
socket.on('user_list', (users) => {
    ...
    users.forEach(u => {
        ...
        if (state.isAdmin && u.username !== state.username && u.username !== 'xp12喵的AI') {
            ...
            // 查看PVP记录按钮
            const pvpCheckBtn = document.createElement('button');
            pvpCheckBtn.className = 'admin-btn';
            pvpCheckBtn.textContent = '📊';
            pvpCheckBtn.title = '查看PVP对局记录';
            pvpCheckBtn.onclick = () => {
                socket.emit('admin_check_pvp', { username: u.username });
            };
            actions.appendChild(pvpCheckBtn);
...
checkPvpBtn.onclick = () => {
    ...
    const username = checkPvpInput.value.trim();
    if (!username) {
        addChat('系统', '❌ 请输入用户名');
        return;
    }
    socket.emit('admin_check_pvp', { username: username });
};
checkPvpInput.onkeydown = (e) => { if (e.key === 'Enter') checkPvpBtn.click(); };
...
console.log('⏳ 真人房间每30分钟限15局，AI房间不限');
</script>
</body>
</html>
'''
...
def handle_create_room(data):
    ...
    # 如果是PVP房间，检查是否超过限制
    if not is_ai:
        can_play, msg = check_pvp_game_limit(username)
        if not can_play:
            emit('error', msg)
            return
...
def handle_place_piece(data):
    ...
    if check_win(room["board"], row, col, color):
        ...
        if room.get("is_ai_room", False):
            ...
            update_ranking(winner_name, loser_name, room_id)
    ...
    if is_board_full(room["board"]):
        ...
        if room.get("is_ai_room", False) and len(players) >= 1:
            ...
            update_ranking_draw(players[0], players[1], room_id)
...
def handle_resign(room_id):
    ...
    if room.get("is_ai_room", False):
        ...
        update_ranking(winner, loser, room_id)
...
# ============ 管理员查询PVP记录 ============
@socketio.on('admin_check_pvp')
def handle_admin_check_pvp(data):
    sid = request.sid
    admin_name = clients[sid].get("username")
    if not admin_name or not is_admin(admin_name):
        emit('error', '权限不足')
    ...
    if target in player_pvp_game_times:
        times = player_pvp_game_times[target]
        current_time = time.time()
        recent = [t for t in times if current_time - t < PVP_WINDOW_SECONDS]
        recent_times = [time.strftime('%H:%M:%S', time.localtime(t)) for t in recent[-10:]]
        
        emit('pvp_player_info', {
            'username': target,
            'games_30min': len(recent),
