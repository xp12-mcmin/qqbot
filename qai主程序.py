...
检测到 LLBot → 杀 LLBot + 杀所有 Python → 自爆
"""
import psutil
import time
import os
...
MAIN_SCRIPT = "llbot_watchdog.py"
START_FLAG = "watchdog_started.flag"
...
def start_main_once():
    if os.path.exists(START_FLAG):
        print("[看门狗A] 主程序已启动过，跳过")
        return
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    script_path = os.path.join(script_dir, MAIN_SCRIPT)
    
    if not os.path.exists(script_path):
        print(f"[看门狗A] 主程序不存在: {script_path}")
        return
